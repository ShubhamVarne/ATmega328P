...
int main(void)
{
	...
	DDRC = 0x01; //we had written 0010 0000 in order to set PB5 as output pin
	...
	while(1){
		...
		/*Here our intention is to connect LED in active low configuration
		 * and blink it. As onboard LED's anode is already connected to 
		 * micor-controller we can't use PB5 LED. 
		 *
		 * We will use PC0 pin of ATmega328P and we will connect LED in active
		 * low configuration to it.
		 *
		 ...
		 *
		 * To make LED ON we will give logic 0 to its cathode
		 * and to make LED OFF we will give logic 1 to its cathode
		 **/
		PORTC ^= 0x01;
		Delay(125000);
	}
